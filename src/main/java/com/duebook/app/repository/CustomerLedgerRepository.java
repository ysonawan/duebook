...
public interface CustomerLedgerRepository extends JpaRepository<CustomerLedger, Long> {
    ...
    @Query("SELECT cl FROM CustomerLedger cl WHERE cl.shop.id IN :shopIds AND cl.entryType = :entryType ORDER BY cl.entryDate DESC, cl.createdAt DESC")
    ...
    @Query("SELECT cl FROM CustomerLedger cl WHERE cl.shop.id IN :shopIds AND cl.customer.id = :customerId AND cl.entryType = :entryType ORDER BY cl.entryDate DESC, cl.createdAt DESC")
    ...
    @Query("SELECT cl FROM CustomerLedger cl WHERE cl.shop.id IN :shopIds AND cl.entryType = :entryType AND CAST(cl.entryDate AS date) >= :startDate AND CAST(cl.entryDate AS date) <= :endDate ORDER BY cl.entryDate DESC, cl.createdAt DESC")
    ...
    @Query("SELECT cl FROM CustomerLedger cl WHERE cl.shop.id IN :shopIds AND cl.customer.id = :customerId AND cl.entryType = :entryType AND CAST(cl.entryDate AS date) >= :startDate AND CAST(cl.entryDate AS date) <= :endDate ORDER BY cl.entryDate DESC, cl.createdAt DESC")
